...
{
    std::vector<std::string> arguments = {"-s", "tt_streams.txt", "-e", "emergency_streams.txt", "-z", "schedule.txt"};
    ASSERT_THROW(io::ProgramOptions{arguments}, std::runtime_error);

    arguments = {"-t", "topology.txt", "-e", "emergency_streams.txt", "-z", "schedule.txt"};
    ASSERT_THROW(io::ProgramOptions{arguments}, std::runtime_error);

    arguments = {"-t", "topology.txt", "-s", "tt_streams.txt", "-z", "schedule.txt"};
    ASSERT_THROW(io::ProgramOptions{arguments}, std::runtime_error);

    arguments = {"-t", "topology.txt", "-s", "tt_streams.txt", "-e", "emergency_streams.txt"};
    ASSERT_THROW(io::ProgramOptions{arguments}, std::runtime_error);
}

...
namespace tsndgm
{
    ...
    public:
        ...
        RTI(Delay max, Delay min = 0, Delay prop = 0, Delay proc = 0) : min(min), max(max), prop(prop), proc(proc) {}

        RTI(FrameSize frame_size, DataRate data_rate, Delay prop = 0, Delay proc = 0, Delay skew = 0) :
            max(frame_size * SECONDS_TO_TICKS(1) / data_rate + skew),
            min(frame_size * SECONDS_TO_TICKS(1) / data_rate - skew), prop(prop), proc(proc)
        {
        }
    ...
    public:
        ...
        MessageStream(const std::shared_ptr<NetworkTopology> &network, nlohmann::json j);

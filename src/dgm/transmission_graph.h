...
namespace tsndgm
{
    ...
    struct TransmissionGraphVertexProperty
    {
        ...
        void shuffle(transmission_graph_t& transmission_graph,
                     TransmissionGraphVertexProperty& other)
        {
        ...
        void invalidate_neighbors(transmission_graph_t& transmission_graph)
        {
            ...
            for (auto parent : JP)
            {
                if (parent.v != prop.src)
                    transmission_graph[parent.v].neighbors_are_valid = false;
            }
        }
    };
    ...
    struct PtrOrientationStatePair
    {
        ...
        void add_edge(E e, transmission_graph_t& transmission_graph)
        {
            equivalence_class.insert(
                {source(e, transmission_graph), target(e, transmission_graph)});
        }
    ...
    static void update_machine_successor(
        ...
        boost::graph_traits<transmission_graph_t>::vertex_descriptor u,
        boost::graph_traits<transmission_graph_t>::vertex_descriptor v)
    {
        auto uv = boost::edge(u, v, transmission_graph).first;
        transmission_graph[u].MS = {v, uv};
        transmission_graph[v].MP = {u, uv};

        transmission_graph[u].FS.clear();
        for (auto& v_parent : transmission_graph[v].JP)
        {
            if (v_parent.v != transmission_graph[boost::graph_bundle].src)
            {
                auto [fuv, found] = boost::edge(u, v_parent.v, transmission_graph);
                if (found)
                {
                    transmission_graph[u].FS.push_back({v_parent.v, fuv});
                    transmission_graph[v_parent.v].FP[v] = {u, fuv};
                }
            }
        }
    }
    ...
    struct TransmissionGraphEdgeProperty
    {
        ...
        void consistent_flip(transmission_graph_t& transmission_graph)
        {
            ...
            {
                auto [uv, exists] = boost::edge(u, v, transmission_graph);
                ...
                if (!exists || transmission_graph[uv].edge_type != disjunctive)
                    continue;
                ...
                V w = u;
                while (transmission_graph[w].MS.has_value() && w != v)
                    w = transmission_graph[w].MS->v;
                if (w != v)
                    continue;

                V first = transmission_graph[u].MP.has_value() ? transmission_graph[u].MP->v : u;
                V last = transmission_graph[v].MS.has_value() ? transmission_graph[v].MS->v : v;
                for (w = first; w != last; w = transmission_graph[w].MS->v)
                {
                    transmission_graph[w].invalidate_neighbors(transmission_graph);
                }
                transmission_graph[last].invalidate_neighbors(transmission_graph);
            }
        }

        bool relates_to(const TransmissionGraphEdgeProperty& other) const
        {
            return state_pair->state.get() == other.state_pair->state.get();
        }

        void merge_equivalence_classes(E other, transmission_graph_t& transmission_graph,
                                       TransmissionGraphProperty& prop)
        {
            for (auto e : state_pair->equivalence_class)
            {
                auto e_new = boost::edge(e.first, e.second, transmission_graph);
                if (e_new.second)
                {
                    transmission_graph[e_new.first].state_pair = transmission_graph[other].state_pair;
                    transmission_graph[other].state_pair->equivalence_class.insert(e);
                }
            }
        }
    };
    ...
    public:
        ...
        NeighborVertexIterator&
        operator=(const transmission_graph_t::in_edge_iterator& other)
        {
            BOOST_STATIC_ASSERT(
        ...
        NeighborVertexIterator&
        operator=(const transmission_graph_t::out_edge_iterator& other)
        {
            BOOST_STATIC_ASSERT(
    ...
    private:
        ...
        transmission_graph_t::in_edge_iterator full_in_iterator;
        transmission_graph_t::out_edge_iterator full_out_iterator;
        NeighborVertex ref;
    };

    template <typename T>
    class NeighborVertexIteratorRange
    {
    public:
    ...
    static auto
    restricted_out_edges(boost::graph_traits<transmission_graph_t>::vertex_descriptor v,
                         const transmission_graph_t& g)
    {
    ...
    static auto
    conjunctive_out_edges(boost::graph_traits<transmission_graph_t>::vertex_descriptor v,
                          const transmission_graph_t& g)
    {
    ...
    static auto
    fifo_out_edges(boost::graph_traits<transmission_graph_t>::vertex_descriptor v,
                   const transmission_graph_t& g)
    {
        return NeighborVertexIteratorRange(&g[v].MS, &g[v].JS, &g[v].FS,
    ...
    static auto
    restricted_in_edges(boost::graph_traits<transmission_graph_t>::vertex_descriptor v,
                        const transmission_graph_t& g)
    {
    ...
    static auto
    conjunctive_in_edges(boost::graph_traits<transmission_graph_t>::vertex_descriptor v,
                         const transmission_graph_t& g)
    {
    ...
    static auto
    fifo_in_edges(boost::graph_traits<transmission_graph_t>::vertex_descriptor v,
                  const transmission_graph_t& g)
    {
        return NeighborVertexIteratorRange(&g[v].MP, &g[v].JP, &g[v].FP,
    ...
    static void print_processing_order(
        const transmission_graph_t& transmission_graph,
        boost::graph_traits<transmission_graph_t>::vertex_descriptor v)
    {
        while (transmission_graph[v].MP.has_value())
            v = transmission_graph[v].MP.value().v;
        while (transmission_graph[v].MS.has_value())
        {
            std::cout << v << " ";
            v = transmission_graph[v].MS.value().v;
        }
        std::cout << v << std::endl;
    }
                      ...
                      const NetworkTopology& network)
    {
        ...
        for (auto vd : boost::make_iterator_range(boost::vertices(transmission_graph)))
        {
            std::cout << vd << ": ";
            print(transmission_graph, network, vd);
            std::cout << ": " << transmission_graph[boost::graph_bundle].crit_cost[vd]
                << ", " << transmission_graph[boost::graph_bundle].slack[vd] << " ("
                << transmission_graph[boost::graph_bundle].crit_pred[vd] << ")"
                << std::endl;
        }
        ...
        for (auto vd : boost::make_iterator_range(boost::vertices(transmission_graph)))
        {
            for (auto& nv : conjunctive_out_edges(vd, transmission_graph))
            {
                print(transmission_graph, network, nv.e);
                std::cout << std::endl;
            }
        }
        ...
        for (auto vd : boost::make_iterator_range(boost::vertices(transmission_graph)))
        {
            if (transmission_graph[vd].MS.has_value())
            {
                print(transmission_graph, network, transmission_graph[vd].MS.value().e);
                std::cout << std::endl;
            }
        }
        ...
        for (auto vd : boost::make_iterator_range(boost::vertices(transmission_graph)))
        {
            for (auto& nv : fifo_out_edges(vd, transmission_graph))
            {
                print(transmission_graph, network, nv.e);
                std::cout << std::endl;
            }
    ...
    static void assert_synchronicity(const transmission_graph_t& transmission_graph)
    {
        typedef boost::graph_traits<transmission_graph_t>::vertex_descriptor V;
        typedef boost::graph_traits<transmission_graph_t>::edge_descriptor E;
        ...
        for (auto& [op, v] : prop.operation_to_vertex)
        {
            ...
            {
                auto e = boost::edge(operations[op.first][i], v, transmission_graph);
                assert((e.second));
                if (transmission_graph[e.first].state() == blocked)
                    break;
            }
            ...
            {
                auto e = boost::edge(operations[op.first][i], v, transmission_graph);
                assert((e.second && transmission_graph[e.first].state() == blocked));
            }
        }

        for (auto& [edge, processing_order] : operations)
        {
            assert((!transmission_graph[processing_order[0]].neighbors_are_valid ||
                !transmission_graph[processing_order[0]].MP.has_value()));
            for (int i = 1; i < processing_order.size(); i++)
            {
                if (!transmission_graph[processing_order[i - 1]].neighbors_are_valid ||
                    !transmission_graph[processing_order[i]].neighbors_are_valid)
                    continue;

                assert((transmission_graph[processing_order[i - 1]].MS->v ==
                    processing_order[i]));
                assert((transmission_graph[processing_order[i]].MP->v ==
                    processing_order[i - 1]));

                for (auto& nv : transmission_graph[processing_order[i]].JP)
                {
                    if (nv.v == prop.src)
                        continue;

                    assert((transmission_graph[nv.v].FP.at(processing_order[i]).v ==
                        processing_order[i - 1]));
                    assert((std::find_if(transmission_graph[processing_order[i - 1]].FS.begin(),
                            transmission_graph[processing_order[i - 1]].FS.end(),
                            [&](auto &nv1) { return nv.v == nv1.v; }) !=
                        transmission_graph[processing_order[i - 1]].FS.end()));
                }
            }
            assert((!transmission_graph[processing_order[processing_order.size() - 1]]
                .neighbors_are_valid ||
                !transmission_graph[processing_order[processing_order.size() - 1]]
                .MS.has_value()));
        }
        ...
        for (V v : boost::make_iterator_range(boost::vertices(transmission_graph)))
        {
            ...
            for (auto& [u, nv] : transmission_graph[v].FP)
                assert((transmission_graph[u].MP->v == nv.v));
            for (auto& nv : transmission_graph[v].FS)
            {
                bool found = false;
                for (auto& [u, nv1] : transmission_graph[nv.v].FP)

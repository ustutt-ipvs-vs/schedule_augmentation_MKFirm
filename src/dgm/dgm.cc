...
namespace tsndgm
{
        ...
        std::map<MessageStreamHandle, RTIMap> rti_updates)
    {
        ...
        committed_transmission_graphs.clear();
    ...
    void DisjunctiveGraphModel::update_rti(MessageStreamHandle ms, RTIMap rti_map)
    {
        ...
        for (auto& [edge, rti] : rti_map)
        {
            ...
            for (auto& nv : transmission_graph[v].JP)
            {
                V u = nv.v;
                Edge edge = transmission_graph[u].edge;
                bool u_contains_ms = std::find(transmission_graph[u].ms_handle.begin(),
                                               transmission_graph[u].ms_handle.end(),
                                               ms) != transmission_graph[u].ms_handle.end();
        ...
        for (auto& [edge, rti] : rti_map)
        {
            V v = prop.operation_to_vertex[{edge, ms}];
            for (E vw :
                 boost::make_iterator_range(boost::out_edges(v, transmission_graph)))
            {
                if (transmission_graph[vw].edge_type != conjunctive)
                    continue;
                V w = target(vw, transmission_graph);
                bool w_contains_ms = std::find(transmission_graph[w].ms_handle.begin(),
                                               transmission_graph[w].ms_handle.end(),
                                               ms) != transmission_graph[w].ms_handle.end();
                if (w_contains_ms)
                {
                    Delay d_max_old = std::accumulate(
                        transmission_graph[v].ms_handle.begin(),
                        transmission_graph[v].ms_handle.end(), (Delay)0,
                    ...
                    if (rti_map[edge].d_max() > d_max_old)
                    {
                        ...
                        transmission_graph[vw].weight += rti_map[edge].d_max() - d_max_old;
                    }
                    else
                    {
                        Edge n_edge = transmission_graph[w].edge;
                        ...
                        transmission_graph[vw].weight += delta_new - delta_old;
                    }
                }
                else
                {
                    transmission_graph[vw].weight +=
        ...
        {
            V v = prop.operation_to_vertex[{edge, ms}];
            for (E uv : boost::make_iterator_range(boost::in_edges(v, transmission_graph)))
            {
                if (transmission_graph[uv].edge_type == fifo)
                {
                    Delay d_min_old = std::accumulate(
                        transmission_graph[v].ms_handle.begin(),
                        transmission_graph[v].ms_handle.end(), std::numeric_limits<Delay>::max(),
                    ...
                    if (rti_map[edge].d_min() < d_min_old)
                    {
                        ...
                        transmission_graph[uv].weight -= rti_map[edge].d_min() - d_min_old;
                    }
                }
            }
        }
    ...
    void DisjunctiveGraphModel::internal_commit_all(size_t index)
    {
        ...
        reversed_dgm_traversal(
            transmission_graph,
            visitor(update_machine_successors_visitor(transmission_graph, updates))
            .root_vertex(transmission_graph[boost::graph_bundle].sink));
        update_machine_successors(updates);
        commit_flips();

        for (size_t i = committed_transmission_graphs.size(); i <= index; i++)
            committed_transmission_graphs.push_back(transmission_graph_t());

        committed_transmission_graphs[index].clear();
        boost::copy_graph(transmission_graph, committed_transmission_graphs[index]);
        committed_transmission_graphs[index][boost::graph_bundle] =
            transmission_graph[boost::graph_bundle];

        for (E e : boost::make_iterator_range(boost::edges(transmission_graph)))
        {
            if (transmission_graph[e].edge_type == disjunctive)
            {
                transmission_graph[e].state_pair->commit(index);
            }
        }
    }
    ...
    void DisjunctiveGraphModel::internal_copy_commit(size_t src_index,
                                                     size_t dst_index)
    {
        for (size_t i = committed_transmission_graphs.size(); i <= dst_index; i++)
            committed_transmission_graphs.push_back(transmission_graph_t());

        committed_transmission_graphs[dst_index].clear();
        boost::copy_graph(committed_transmission_graphs[src_index],
                          committed_transmission_graphs[dst_index]);
        committed_transmission_graphs[dst_index][boost::graph_bundle] =
            committed_transmission_graphs[src_index][boost::graph_bundle];

        for (E e : boost::make_iterator_range(boost::edges(transmission_graph)))
        {
            if (transmission_graph[e].edge_type == disjunctive)
            {
                transmission_graph[e].state_pair->copy_commit(src_index, dst_index);
            }
        }
    }
    ...
    void DisjunctiveGraphModel::restore_flips(size_t n)
    {
        ...
        for (int k = n; k > 0; k--, flip_log.pop_front())
        {
            ...
            transmission_graph[uv].consistent_flip(transmission_graph);
        }
    ...
    void DisjunctiveGraphModel::internal_restore_commit(size_t index, bool swap)
    {
        ...
        if (index > committed_transmission_graphs.size())
            throw std::runtime_error("commit does not exist");

        if (boost::num_vertices(committed_transmission_graphs[index]) > 0)
        {
            transmission_graph.clear();
            if (swap)
            {
                std::swap(committed_transmission_graphs[index], transmission_graph);
            }
            else
            {
                boost::copy_graph(committed_transmission_graphs[index], transmission_graph);
                transmission_graph[boost::graph_bundle] =
                    committed_transmission_graphs[index][boost::graph_bundle];
            }
            ...
            for (E e : boost::make_iterator_range(boost::edges(transmission_graph)))
            {
                if (transmission_graph[e].edge_type == disjunctive)
                {
                    transmission_graph[e].state_pair->restore_commit(index, swap);
                }
            }
        }
        valid_crit_path = false;
    ...
    void DisjunctiveGraphModel::update_machine_successors(std::map<V, V> updates)
    {
        ...
        {
            if (!transmission_graph[u].MS.has_value() || transmission_graph[u].MS->v == v)
                continue;

            V old_succ = transmission_graph[u].MS->v;
            transmission_graph[old_succ].MP = {};
            for (auto& old_succ_parent : transmission_graph[old_succ].JP)
            {
                transmission_graph[old_succ_parent.v].FP.erase(old_succ);
            }
        }
        for (auto& [u, v] : updates)
        {
            if (v != 0)
            {
                update_machine_successor(transmission_graph, u, v);
            }
            else
            {
                transmission_graph[u].MS = {};
                transmission_graph[u].FS.clear();
            }
            transmission_graph[u].neighbors_are_valid = true;
        }
    }

    std::vector<boost::graph_traits<transmission_graph_t>::vertex_descriptor>
    DisjunctiveGraphModel::get_processing_order(transmission_graph_t& g, Edge edge)
    {
    ...
    std::vector<boost::graph_traits<transmission_graph_t>::vertex_descriptor>
    DisjunctiveGraphModel::get_processing_order(transmission_graph_t& g, V v)
    {
    ...
    void DisjunctiveGraphModel::complete_flip(std::list<E>& edges, bool combined)
    {
        ...
        else
        {
            for (E& e : edges)
            {
                if (transmission_graph[e].state() == blocked)
                    e = rev_edge(e);
                else if (transmission_graph[e].edge_type == fifo)
            ...
            for (E e : edges)
            {
                assert((transmission_graph[e].edge_type != conjunctive));
                transmission_graph[e].consistent_flip(transmission_graph);
                flipped_edges.insert(transmission_graph[e].state_pair->state.get());
        ...
        const std::set<V>& shuffled_operations, std::optional<E> uv)
    {
        ...
        if (uv.has_value())
        {
            ...
            required_flips_classes.insert(transmission_graph[*uv].state_pair->state.get());

            V u = source(*uv, transmission_graph), v = target(*uv, transmission_graph), w = u;
            while (w != v && transmission_graph[w].neighbors_are_valid)
            {
                E wv = edge(w, v);
        ...
        try
        {
            do
            {
                ...
                reversed_dgm_traversal(
                    transmission_graph,
                    visitor(complete_flip_visitor(transmission_graph, flipped_edges,
                                                 ...
                                                 bool fix_cycles)
    {
        ...
        transmission_graph[boost::graph_bundle].is_zips_selection = false;
        if (fix_cycles)
            renew_descriptors();
    }
        ...
        std::set<V>& shuffled_operations, bool fix_cycles)
    {
        ...
        {
            closed = true;
            auto& equivalence_class = transmission_graph[uv].state_pair->equivalence_class;
            for (auto& [u, v] : equivalence_class)
            {
                ...
                else
                {
                    violation = std::find_if(
                        equivalence_class.begin(), equivalence_class.end(), [&](auto& e)
                        {
                            auto uw = boost::edge(u, e.first, transmission_graph);
                            if (!uw.second ||
                                transmission_graph[uw.first].edge_type != disjunctive)
                                return false;
                            return v == e.second &&
                ...
                flipped_edges.erase(transmission_graph[uw].state_pair->state.get());
                flipped_edges.erase(transmission_graph[uw].state_pair->reversed_state.get());
                transmission_graph[uw].merge_equivalence_classes(uv, transmission_graph, prop);
                transmission_graph[rev_edge(uw)].merge_equivalence_classes(
                    rev_edge(uv), transmission_graph, prop);
                closed = false;
                break;
            }
        }
        ...
        for (auto& [u, v] : transmission_graph[uv].state_pair->equivalence_class)
        {
            if (transmission_graph[u].edge != transmission_graph[v].edge)
            {
                std::erase_if(transmission_graph[v].FP, [&](const auto& item)
                {
                    auto const& [w, nv] = item;
                    return transmission_graph[uv].state_pair->equivalence_class.contains({u, w});
                });
                continue;
            }
            remove_fifo_edges(u, v);
            remove_fifo_edges(v, u);
            for (MessageStreamHandle ms_handle : transmission_graph[v].ms_handle)
            {
                prop.operation_to_vertex[Operation(transmission_graph[v].edge, ms_handle)] = u;
            }
            transmission_graph[u].shuffle(transmission_graph, transmission_graph[v]);
            shuffled_operations.insert(u);
            for (MessageStreamHandle ms_handle : transmission_graph[u].ms_handle)
            {
        ...
        for (auto& [u, v] : transmission_graph[uv].state_pair->equivalence_class)
        {
            ...
            transmission_graph[u].invalidate_neighbors(transmission_graph);
            if (transmission_graph[u].MP.has_value())
                transmission_graph[transmission_graph[u].MP->v].invalidate_neighbors(transmission_graph);
            if (transmission_graph[v].MS.has_value())
                transmission_graph[transmission_graph[v].MS->v].invalidate_neighbors(transmission_graph);

            // Update outgoing conjunctive edges by updating their weights
            // along with JS and JP
            for (auto& [w, vw] : conjunctive_out_edges(v, transmission_graph))
            {
                if (w != prop.sink && transmission_graph[w].ms_handle.empty())
                    continue;

                Edge edge = transmission_graph[u].edge;
                Edge n_edge = transmission_graph[w].edge;
                NeighborVertex uw;
                auto u_JS = std::find_if(transmission_graph[u].JS.begin(),
                                         transmission_graph[u].JS.end(), [&](auto& nv)
                                         {
                                             return nv.v == w || transmission_graph[nv.v].edge ==
                                                 transmission_graph[w].edge;
                                         });
                if (u_JS != transmission_graph[u].JS.end())
                {
                    if (w == prop.sink)
                        transmission_graph[w].JP.remove({v, vw});

                    uw = *u_JS;
                }
                else
                {
                    assert((!boost::edge(u, w, transmission_graph).second));

                    uw = {w, boost::add_edge(u, w, transmission_graph[vw], transmission_graph).first};
                    transmission_graph[w].JP.remove({v, vw});
                    transmission_graph[u].JS.push_back({w, uw.e});
                    transmission_graph[w].JP.push_back({u, uw.e});
                }
            }

            for (auto& [w, uw] : conjunctive_out_edges(u, transmission_graph))
            {
                Edge edge = transmission_graph[u].edge;
                Edge n_edge = transmission_graph[w].edge;
                transmission_graph[uw].weight = 0;
                if (network->has_multiple_subcarriers(edge))
                {
                    Delay d_wireline = 0;
                    for (auto& handle : transmission_graph[u].ms_handle)
                    {
                        transmission_graph[uw].weight =
                            std::max(transmission_graph[uw].weight,
                                     prop.streams[handle].rti_map[edge].d_max());
                        d_wireline += prop.streams[handle].rti_map[edge].d_wireline();
                    }
                    transmission_graph[uw].weight += d_wireline;
                }
                else
                {
                    ...
                    for (auto& handle : transmission_graph[u].ms_handle)
                    {
                        if (std::find(transmission_graph[w].ms_handle.begin(),
                                      transmission_graph[w].ms_handle.end(),
                                      handle) != transmission_graph[w].ms_handle.end() &&
                            !network->has_multiple_subcarriers(n_edge))
                        {
                            ...
                            transmission_graph[uw].weight += delay.second;
                        }
                        else
                        {
                    ...
                    transmission_graph[uw].weight += max_delay.first - max_delay.second;
                }
            }
            ...
            for (auto& [w, wv] : conjunctive_in_edges(v, transmission_graph))
            {
                if (w != prop.src && transmission_graph[w].ms_handle.empty())
                    continue;

                auto u_JP = std::find_if(transmission_graph[u].JP.begin(),
                                         transmission_graph[u].JP.end(), [&](auto& nv)
                                         {
                                             return nv.v == w || transmission_graph[nv.v].edge ==
                                                 transmission_graph[w].edge;
                                         });
                if (u_JP == transmission_graph[u].JP.end())
                {
                    assert((!boost::edge(w, u, transmission_graph).second));

                    E wu = boost::add_edge(w, u, transmission_graph[wv], transmission_graph).first;
                    transmission_graph[u].JP.push_back({w, wu});
                    transmission_graph[w].JS.push_back({u, wu});
                    transmission_graph[w].JS.remove({v, wv});
                    transmission_graph[w].FP.erase(v);
                }
                else if (w == prop.src)
                {
                    transmission_graph[w].JS.remove({v, wv});
            ...
            {
                V w = target(vw, transmission_graph);
                if (w == u || transmission_graph[vw].edge_type == conjunctive)
                    continue;

                auto [uw, exists] = boost::edge(u, w, transmission_graph);
                if (!exists || transmission_graph[vw].relates_to(transmission_graph[uw]) ||
                    transmission_graph[vw].relates_to(transmission_graph[rev_edge(uw)]))
                    continue;

                transmission_graph[uw].weight +=
                    transmission_graph[fifo_to_disjunctive_edge(vw)].weight;
            }
            ...
            for (E wv : boost::make_iterator_range(boost::in_edges(v, transmission_graph)))
            {
                V w = source(wv, transmission_graph);
                if (transmission_graph[uv].relates_to(transmission_graph[wv]) ||
                    transmission_graph[wv].edge_type != fifo)
                    continue;

                // FIFO edge w -> u does not necessarily exist. Thus, add this edge if
                // needed and adjust the weights accordingly.
                auto wu = boost::edge(w, u, transmission_graph);
                if (!wu.second)
                {
                    wu = boost::add_edge(w, u, transmission_graph[wv], transmission_graph);
                    transmission_graph[wu.first].state_pair->add_edge(wu.first, transmission_graph);
                }
                else
                {
                    transmission_graph[wu.first].weight =
                        std::min(transmission_graph[wu.first].weight, transmission_graph[wv].weight);
                }
                ...
                if (transmission_graph[w].FS.remove_if(
                    [v](NeighborVertex& nv) { return nv.v == v; }) > 0)
                {
                    assert((u_JS != transmission_graph[u].JS.end() && w != prop.sink));

                    transmission_graph[w].FS.push_back({u, wu.first});
                    transmission_graph[u].FP[(*u_JS).v] = {w, wu.first};
                }
            }
        }
        ...
        for (auto& [u, v] : transmission_graph[uv].state_pair->equivalence_class)
        {
            ...
            {
                V w = target(vw, transmission_graph);
                if (w == u || transmission_graph[vw].edge_type != disjunctive)
                    continue;
                ...
                if (flipped_edges.erase(transmission_graph[vw].state_pair->state.get()) > 0)
                    flipped_edges.insert(transmission_graph[uw].state_pair->state.get());

                if (transmission_graph[vw].state() != transmission_graph[uw].state())
                {
                    if (transmission_graph[uw].state() == blocked)
                        transmission_graph[rev_edge(uw)].consistent_flip(transmission_graph);
                    else
                        transmission_graph[rev_edge(vw)].consistent_flip(transmission_graph);

                    // remove vw from flipped_edges (if exists) and add uw either way
                    flipped_edges.erase(transmission_graph[vw].state_pair->reversed_state.get());
                    flipped_edges.insert(
                        transmission_graph[uw].state_pair->reversed_state.get());
                }

                transmission_graph[vw].merge_equivalence_classes(uw, transmission_graph, prop);
            }
            ...
            for (E wv : boost::make_iterator_range(boost::in_edges(v, transmission_graph)))
            {
                V w = source(wv, transmission_graph);
                if (transmission_graph[uv].relates_to(transmission_graph[wv]) ||
                    transmission_graph[wv].edge_type != disjunctive)
                    continue;

                auto [wu, exists] = boost::edge(w, u, transmission_graph);
                if (!exists || transmission_graph[wv].relates_to(transmission_graph[wu]) ||
                    transmission_graph[wv].relates_to(transmission_graph[rev_edge(wu)]))
                    continue;

                // replace wv (which will be invalid after merging) with wu in
                // flipped_edges (if wv is contained in flipped_edges)
                if (flipped_edges.erase(transmission_graph[wv].state_pair->state.get()) > 0)
                    flipped_edges.insert(transmission_graph[wu].state_pair->state.get());
                if (flipped_edges.erase(
                    transmission_graph[wv].state_pair->reversed_state.get()) > 0)
                    flipped_edges.insert(
                        transmission_graph[wu].state_pair->reversed_state.get());

                transmission_graph[wv].merge_equivalence_classes(wu, transmission_graph, prop);
            }
        }

        // remove uv from flipped_edges
        flipped_edges.erase(transmission_graph[uv].state_pair->state.get());
        flipped_edges.erase(transmission_graph[uv].state_pair->reversed_state.get());
        ...
        for (auto& [u, v] : transmission_graph[uv].state_pair->equivalence_class)
        {
            if (transmission_graph[u].edge == transmission_graph[v].edge)
            {
                ...
                if (!transmission_graph[v].MP.has_value() || transmission_graph[v].MP->v != u)
                {
                    transmission_graph[u].MP = {};
                    for (auto& u_parent : transmission_graph[u].JP)
                    {
                        transmission_graph[u_parent.v].FP.erase(u);
                    }
                }
                if (transmission_graph[v].MS.has_value() && transmission_graph[v].MS->v != u)
                {
                    V w = transmission_graph[v].MS->v;
                    transmission_graph[w].MP = {};
                    for (auto& w_parent : transmission_graph[w].JP)
                    {
                        transmission_graph[w_parent.v].FP.erase(w);
                    }
                }

                boost::clear_vertex(v, transmission_graph);
                transmission_graph[v].clear();
            }
        }
        ...
        for (E e : boost::make_iterator_range(boost::edges(transmission_graph)))
        {
            std::erase_if(transmission_graph[e].state_pair->equivalence_class, [&](auto uv)
            {
                auto& [u, v] = uv;
                return !boost::edge(u, v, transmission_graph).second;
            });
        }
    ...
    {
        TransmissionGraphProperty& prop = transmission_graph[boost::graph_bundle];
        TransmissionGraphProperty& z_prop =
            committed_transmission_graphs[0][boost::graph_bundle];
        ...
        for (auto& [edge, streams] : prop.edge_to_streams)
        {
            ...
            while (u.has_value())
            {
                for (auto& s : transmission_graph[*u].ms_handle)
                {
                    processing_order[edge].push_back(
                        z_prop.operation_to_vertex[Operation(edge, s)]);
                }

                u = transmission_graph[*u].MS.has_value()
                        ? transmission_graph[*u].MS->v
                        : std::optional<V>();
            }
        }

        // Copy initial_transmission_graph to transmission_graph and reset equivalence
        // classes.
        internal_restore_commit(initial, false);
    ...
    {
        TransmissionGraphProperty& prop = transmission_graph[boost::graph_bundle];
        for (auto [u, v] : transmission_graph[e].state_pair->equivalence_class)
        {
            E uv = edge(u, v);
            if (transmission_graph[uv].edge_type == fifo)
                continue;

            for (auto u_ms : transmission_graph[u].ms_handle)
            {
                for (auto& u_ms_listener : prop.streams[u_ms].route->get_listeners())
                {
                    Delay u_jitter = compute_jitter(u_ms, u_ms_listener);
                    for (auto v_ms : transmission_graph[v].ms_handle)
                    {
    ...
    void DisjunctiveGraphModel::remove_fifo_edges(V u, V v)
    {
        ...
        for (auto& nv : transmission_graph[v].JP)
        {
            auto search = boost::edge(u, nv.v, transmission_graph);
            if (search.second)
            {
                boost::remove_edge(search.first, transmission_graph);
                auto search = transmission_graph[nv.v].FP.find(v);
                if (search != transmission_graph[nv.v].FP.end() && search->second.v == u)
                {
                    transmission_graph[nv.v].FP.erase(v);
                }
            }
        }
    }
    ...
    void DisjunctiveGraphModel::resize_properties()
    {
        ...
        prop.cycle_pred.resize(boost::num_vertices(transmission_graph));
        ...
        for (const TreeRouteHop& hop : *prop.streams[handle].route)
        {
            ...
            if (!hop.parent->is_root())
            {
                ...
                E e = boost::add_edge(
                        ...
                        transmission_graph)
                    .first;
                transmission_graph[v].JP = {{v_parent, e}};
                transmission_graph[v_parent].JS.push_back({v, e});
            }
            else
            {
                E e = boost::add_edge(
                        ...
                        transmission_graph)
                    .first;
                transmission_graph[v].JP = {{prop.src, e}};
                transmission_graph[prop.src].JS.push_back({v, e});
            }
            ...
            if (hop.is_leaf())
            {
                E e = boost::add_edge(v, prop.sink,
                                      ...
                                      transmission_graph)
                    .first;
                transmission_graph[v].JS = {{prop.sink, e}};
                transmission_graph[prop.sink].JP.push_back({v, e});
            }
            ...
            else
            {
                ...
                for (MessageStreamHandle other : edge_search->second)
                {
                    ...
                    TreeRouteHop *v_hop_parent = hop.parent,
                                 *u_hop_parent = transmission_graph[u].hop.front()->parent;
                        ...
                        v_hop_parent->edge == u_hop_parent->edge)
                    {
                        ...
                        E e_parent = boost::edge(v_parent, u_parent, transmission_graph).first,
                          e_rev_parent = boost::edge(u_parent, v_parent, transmission_graph).first;

                        states = std::make_pair(transmission_graph[e_parent].state_pair,
                                                transmission_graph[e_rev_parent].state_pair);
                    }
                    else
                    {
                    ...
                    if (!hop.parent->is_root())
                    {
                        ...
                        fuv = boost::add_edge(u, v_parent, {weight, states.second, fifo},
                                              transmission_graph)
                            .first;
                        transmission_graph[fuv].state_pair->add_edge(fuv, transmission_graph);
                    }
                    ...
                    if (!u_hop_parent->is_root())
                    {
                        ...
                        fvu = boost::add_edge(v, u_parent, {weight, states.first, fifo},
                                              transmission_graph)
                            .first;
                        transmission_graph[fvu].state_pair->add_edge(fvu, transmission_graph);
                    }
                    ...
                    E vu = boost::add_edge(v, u, {weight, states.first, disjunctive},
                                           transmission_graph)
                        .first;
                    transmission_graph[vu].state_pair->add_edge(vu, transmission_graph);
                    ...
                    E uv = boost::add_edge(u, v, {weight, states.second, disjunctive},
                                           transmission_graph)
                        .first;
                    transmission_graph[uv].state_pair->add_edge(uv, transmission_graph);
                    prop.equivalence_class_representative[states.second->state.get()] = uv;

                    if (!transmission_graph[u].MP.has_value())
                    {
                        transmission_graph[v].MS = {u, vu};
                        transmission_graph[u].MP = {v, vu};

                        if (!u_hop_parent->is_root())
                        {
                            transmission_graph[v].FS.push_back({u_parent, fvu});
                            transmission_graph[u_parent].FP[u] = {v, fvu};
                        }
                    }
                }
    ...
    void DisjunctiveGraphModel::encode(std::vector<unsigned int>& buf,
                                       transmission_graph_t& g, OffsetMap& offset_map)
    {
        auto& prop = g[boost::graph_bundle];
        buf.clear();
        offset_map.clear();
    ...
    {
        auto& prop = transmission_graph[boost::graph_bundle];
                ...
                if (buf[offset + len] == SHUFFLE_SEPARATOR)
                {
                    ...
                    while (buf[offset + len] != SHUFFLE_SEPARATOR)
                    {
                        ...
                        if (u != v)
                        {
                            ...
                            if (transmission_graph[v].ms_handle.empty())
                                v = u;
                        }
                        len++;
                    }
        ...
        const std::vector<V>& operations)
    {
        assert((std::accumulate(operations.begin(), operations.end(), 0,
                [&](size_t s, V v) {
                return s + transmission_graph[v].ms_handle.size();
                }) ==
            transmission_graph[boost::graph_bundle]
            .edge_to_streams[transmission_graph[operations.front()].edge]
            .size()));
        ...
        for (int i = 0; i < operations.size(); i++)
        {
            ...
            for (int j = i + 1; j < operations.size(); j++)
            {
                ...
                E e = this->edge(u, v);
                if (transmission_graph[e].state() == blocked)
                    std::swap(*transmission_graph[e].state_pair->state,
                              *transmission_graph[e].state_pair->reversed_state);
                if (j == i + 1)
                    update_machine_successor(transmission_graph, u, v);
            }
            transmission_graph[u].neighbors_are_valid = true;
    ...
    {
        for (V v : boost::make_iterator_range(boost::vertices(transmission_graph)))
        {
            auto& v_prop = transmission_graph[v];

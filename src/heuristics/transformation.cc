...
void Transformation::compute_best_permutation(V v) {
  ...
  auto int_phase_result = dgm.critical_path(this->type);
  std::pair<Delay, int> min_d = {int_phase_result.objective, -1};
  for (int j = 0; j < processing_order.size(); j++) {
    ...
    if (this->accept(processing_order, min_d, int_phase_result, div_result, i,
                     j))
...
void RandomCriticalPathTransformation::transform(int k) {
  ...
  CriticalPath::Result res = dgm.critical_path(this->type);

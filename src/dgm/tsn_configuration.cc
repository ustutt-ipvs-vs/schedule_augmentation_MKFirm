...
namespace tsndgm
{
    void TSNConfiguration::compute()
    {
        ...
        // TODO write new reversed_dgm_traversal function
        ...
        for (auto& entry : gcl_config)
        {
            ...
            gate.offset =
                prop.hyperperiod - std::accumulate(std::begin(gate.durations),
                                                   std::end(gate.durations), 0.0);
        ...
        {
            MessageStream stream = prop.streams[ms];
    ...
    void TSNConfiguration::dump(std::filesystem::path out)
    {
        ...
        {
            if (prop.streams[ms].name != "")
            {
                j["INIT"][prop.streams[ms].name] = initial_transmission_config[ms];
            }
            else
            {
        ...
        for (auto& entry : gcl_config)
        {
            ...
            std::string egress_port =
                "[" + topology.get_device_property(edge.first).name + "," +
        ...
        for (auto& entry : psfp_config)
        {
            ...
            for (auto& psfp_gate : psfp_gates)
            {
                ...
                j["PSFP"][v_name].push_back({
                    ...
                    {"open", psfp_gate.open},
                    {"close", psfp_gate.close}
                });
                for (auto ms : psfp_gate.streams)
                {
                    if (prop.streams[ms].name != "")
                    {
                        j["PSFP"][v_name][i]["streams"].push_back(prop.streams[ms].name);
                    }
                    else
                    {
        ...
        V v, const transmission_graph_t& transmission_graph)
    {
        ...
        for (MessageStreamHandle ms : transmission_graph[v].ms_handle)
        {
            ...
            // and the radio link
            open_duration += RTI(prop.streams[ms].frame_size, rate, prop_delay).d_trans_max();

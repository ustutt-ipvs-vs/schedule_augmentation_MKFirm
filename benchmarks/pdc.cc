...
int main(int argc, char **argv) {
  if (argc != 7) {
    std::cout << "Usage: ./adaptive_scheduling <talkers> <listeners> "
                 "<wireless_streams> <cross_traffic> <timeout> <pdc>"
              << std::endl;
    exit(0);
  }
  ...
  for (int stream = 0; stream < streams; stream++) {
    ...
    if (pdc == 0)
      ...
      rti_map[Edge(0, ho)] = RTI(WIRELESS_UL_DMAX, WIRELESS_UL_DMAX);
  ...
  for (int stream = 0; stream < streams; stream++) {
    ...
    if (pdc == 0)
      ...
      rti_map[Edge(ho, 0)] = RTI(WIRELESS_DL_DMAX, WIRELESS_DL_DMAX);
  ...
  TabuSearchConfig config{
      ...
      CompressionConfig(false, TerminationConfig(240, bound),
